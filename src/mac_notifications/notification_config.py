...
class NotificationConfig:
    ...
    @staticmethod
    def c_compliant(a_str: str) -> Optional[str]:
        return ''.join(filter(lambda x: str.isalnum or str.isspace, a_str)) if a_str else None

    def to_json_notification(self) -> "JSONNotificationConfig":
        return JSONNotificationConfig(
            title=NotificationConfig.c_compliant(self.title),
